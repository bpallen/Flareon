...
#define GFFT_PI 3.1415926535897932
...
namespace gfft {
	...
	struct SinCosSeries {
	   static double value() {
		  return 1-(A*GFFT_PI/B)*(A*GFFT_PI/B)/M/(M+1)
	...
	////// template class Sin
	// compile-time calculation of sin(A*GFFT_PI/B) function
	...
	struct Sin<B,A,float> {
	   static float value() {
		  return (A*GFFT_PI/B)*SinCosSeries<2,24,B,A>::value();
	   }
	};
	template<unsigned B, unsigned A>
	struct Sin<B,A,double> {
	   static double value() {
		  return (A*GFFT_PI/B)*SinCosSeries<2,34,B,A>::value();
	   }
	};

	////// template class Cos
	// compile-time calculation of cos(A*GFFT_PI/B) function
